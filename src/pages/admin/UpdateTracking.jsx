// ===== frontend/src/pages/admin/UpdateTracking.jsx =====
...
const UpdateTracking = () => {
  ...
  const [formData, setFormData] = useState({ status: '', paid: false, image: null });
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      try {
        ...
        setFormData({ status: res.data.status, paid: res.data.paid, image: null });
  ...
  return (
    <div className="max-w-xl mx-auto py-16 px-6">
      ...
      <form onSubmit={handleSubmit} className="grid gap-6">
        <input type="text" placeholder="Status" value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })} className="border p-3 rounded" />
        ...
        {currentImageUrl && (
          <div className="text-sm">
            ...
            <img src={currentImageUrl.startsWith('http') ? currentImageUrl : `http://localhost:5000${currentImageUrl}`} alt="Current Package" className="w-32 h-32 object-cover rounded border" />
          </div>
        )}
